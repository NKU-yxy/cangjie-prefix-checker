 ...
 *
 * Modified 2026-08-14 by the project3230617-388044 team: lossless interval-valued Earley
 * completion relations, ranged worklists, and finalized-history range summaries.
 */
...
/*!
 * \brief Completion history with lossless interval compression of rule start positions.
 *
 * Earley parent states that differ only in rule_start_pos are right-language distinct, so the
 * positions cannot be discarded.  This container stores the exact finite position set as disjoint
 * intervals and materializes individual ParserStates only for completion processing.  All other
 * ParserState fields and the referenced rule id remain unchanged.
 */
class CompletableStateHistory {
 public:
  using StatePair = std::pair<int32_t, ParserState>;
  struct StateRange {
    int32_t ref_rule_id;
    ParserState state;
    int32_t rule_start_pos_end;
  };

  int32_t size() const { return static_cast<int32_t>(rows_.size()); }
  void PushBack(const std::vector<StatePair>& states);
  void PushBackInLatestRow(const StatePair& state);
  void PushRangeInLatestRow(
      int32_t ref_rule_id, const ParserState& state, int32_t rule_start_pos_end
  );
  void PopBack(int32_t count);
  void FinalizeLatestRow();

  std::vector<StatePair> ExpandRow(int32_t position) const;
  std::vector<StatePair> BackExpanded() const { return ExpandRow(size() - 1); }
  std::vector<StateRange> BackRanges() const { return rows_.back().states; }
  void PushBackMergedRanges(
      const std::vector<StateRange>& lhs, const std::vector<StateRange>& rhs
  );
  std::vector<StateRange> MatchingRanges(int32_t position, int32_t ref_rule_id) const;
  std::vector<StateRange> MatchingRangesAcrossRows(
      int32_t position_begin, int32_t position_end, int32_t ref_rule_id
  ) const;

  void CopyRuleStatesToLatest(
      int32_t source_position, int32_t source_rule_id, int32_t target_rule_id
  );
  void CopyRuleStatesRangeToLatest(
 ...
 private:
  ...
  static bool SameCore(const ParserState& lhs, const ParserState& rhs);
  ...
  static void FinalizeRow(PackedRow* row);
  static std::vector<StateRange> MatchingRangesInRow(
      const PackedRow& row, int32_t ref_rule_id
  );
  void AppendRangeToLatest(
      int32_t ref_rule_id, const ParserState& state, int32_t begin, int32_t end
  );
  void MaybeBuildBlockSummary();

  static constexpr int32_t kBlockSize = 32;
  std::vector<PackedRow> rows_;
  // level 0 contains kBlockSize-row summaries; each higher level summarizes two children.
  std::vector<std::vector<PackedRow>> block_levels_;
};

struct QueuedParserState {
  ParserState state;
  int32_t rule_start_pos_end;
};
...
/*! \brief Delta tracker for exact interval-valued Earley state relations. */
class RangeRepeatDetector {
 public:
  std::vector<QueuedParserState> InsertUnvisited(
 ...
 private:
  ...
  std::unordered_map<ParserState, std::vector<Interval>, StateCoreHash, StateCoreEqual> visited_;
};

/*! \brief FIFO worklist that coalesces still-pending deltas of the same state core. */
class RangeWorkQueue {
 public:
  ...
  void Push(const QueuedParserState& delta, int32_t current_position);
  Item PopFront();
...
class EarleyParser {
 ...
 protected:
   ...
   */
  CompletableStateHistory rule_id_to_completable_states_;
  ...
  RangeWorkQueue tmp_process_state_queue_;

  /*! \brief The class is used to check if a state has been added into the queue. */
  RangeRepeatDetector tmp_states_visited_in_queue_;

  bool processing_range_ = false;
  int32_t processing_range_begin_ = 0;
  int32_t processing_range_end_ = 0;
   ...
   */
  void EnqueueRange(const ParserState& state, int32_t rule_start_pos_end);
  void AppendScannableRange(const ParserState& state, int32_t rule_start_pos_end);
  void StoreCompletableState(int32_t ref_rule_id, const ParserState& state);
  ...
  void EnqueueWithoutProcessing(const ParserState& state) {
    const int32_t end =
        ...
        ? processing_range_end_
        : state.rule_start_pos;
    for (const auto& delta : tmp_states_visited_in_queue_.InsertUnvisited(state, end)) {
      AppendScannableRange(delta.state, delta.rule_start_pos_end);
    }
  }
