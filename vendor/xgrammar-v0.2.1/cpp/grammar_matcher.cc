 ...
 *
 * Modified 2026-08-14 by the project3230617-388044 team: preserve ranged completion snapshots
 * when merging atomic-token and byte paths.
 */
...
bool GrammarMatcher::Impl::AcceptToken(int32_t token_id, bool debug_print) {
  ...
  std::vector<CompletableStateHistory::StateRange> atomic_completable;
  ...
  if (atomic_success) {
    ...
    atomic_completable = rule_id_to_completable_states_.BackRanges();
  ...
  if (atomic_success && !byte_path_success) {
    ...
    if (token.empty()) {
      ...
      rule_id_to_completable_states_.PushBackMergedRanges(atomic_completable, {});
      ...
      const auto byte_completable = rule_id_to_completable_states_.BackRanges();
      bool byte_completed = is_completed_.back();
      PopLastStates(1);

      scanable_state_history_.PushBack(merged);
      rule_id_to_completable_states_.PushBackMergedRanges(
          byte_completable, atomic_completable
      );
